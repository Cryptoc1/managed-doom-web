...
public sealed class Renderer
{
    ...
    public void Render( Doom doom, in Span<byte> destination, Fixed frameFrac )
    {
        if( doom.Wiping )
        {
    ...
    private void RenderWipe( Doom doom, in Span<byte> destination )
    {
    ...
    private void WriteData( uint[] colors, in Span<byte> destination )
    {
        var data = screen.Data;
        var buffer = MemoryMarshal.Cast<byte, uint>( destination );
        for( var i = 0; i < buffer.Length; i++ )
        {
            buffer[ i ] = colors[ data[ i ] ];
        }
    }
